...
{
    public partial class UserControlPendingBill : UserControl
    {
        Bill bill;
        public UserControlPendingBill(Bill data)
        {
            InitializeComponent();
            bill = data;
        }
        ...
        private void UserControlPendingBill_Load(object sender, EventArgs e)
        {
            ...
            this.labelAddress.Text = "Address: " + bill.getAddress();
        }

        private void buttonCancel_Click(object sender, EventArgs e)
        {
            FormCancelMessage form = new FormCancelMessage(bill);
            form.ShowDialog();
            if(bill.getStatus() == 3)
            {
                this.Dispose();
